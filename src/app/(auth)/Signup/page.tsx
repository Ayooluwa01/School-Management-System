...
export default function SignupPage() {
  ...
  const [formData, setFormData] = useState({
    admin: { name: "", email: "", phone: "", password: "", role: "SUPER_ADMIN" },
    school: { 
      schoolId: "", 
      name: "", phone: "", email: "", 
      country: "", address: "" 
  ...
  useEffect(() => {
    ...
    if (name && name.length >= 3) {
      ...
      if(cleanName.length >= 3) {
        ...
        const autoId = `${prefix}|${randomIdSeed.current}|SCH`;
        if (formData.school.schoolId !== autoId) {
          setFormData(prev => ({ ...prev, school: { ...prev.school, schoolId: autoId }}));
        }
      }
    }
  }, [formData.school.name, formData.school.schoolId]);
          ...
          <div className="flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-zinc-100">
            ...
            <div className={`transition-all duration-500 ${step === 4 ? 'md:w-1/2 md:pr-10 pb-8 md:pb-0' : 'w-full'}`}>
              ...
              {step === 1 && (
                <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-500">
                  ...
                  <div className="space-y-1.5 opacity-60">
                    <label className="text-[9px] sm:text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">Account Role</label>
                    <div className="relative">
                      <ShieldCheck className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-600" size={18} />
                      <input disabled value="SUPER_ADMIN" className="w-full bg-zinc-50 border border-zinc-200 rounded-xl sm:rounded-2xl pl-11 py-3.5 sm:py-4 text-zinc-500 text-sm font-bold" />
                    </div>
                  </div>
                ...
                <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-500">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input disabled value={formData.school.schoolId || "Generating ID..."} className="w-full bg-indigo-50/50 border border-indigo-100 rounded-xl sm:rounded-2xl pl-11 py-3.5 sm:py-4 text-indigo-700 text-sm font-mono font-bold" />
                    </div>
                  </div>
              ...
              {step === 3 && (
                <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-500">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <DateInput label="Session End" selected={formData.academic.sessionEnd} onChange={(date:any) => updateForm('academic', 'sessionEnd', date)} />
                  </div>
                  <div className="pt-4 border-t border-zinc-100 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <DateInput label="Term End" selected={formData.academic.termEnd} onChange={(date:any) => updateForm('academic', 'termEnd', date)} />
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="pt-8 space-y-4">
                  <button onClick={()=>step === 4 ? undefined : setStep(step+1)} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-4 rounded-xl sm:rounded-[20px] font-black text-sm transition-all active:scale-[0.98] flex items-center justify-center gap-2 shadow-lg shadow-indigo-100">
                      {step === 4 ? "Sign into Dashboard" : "Continue Registration"} 
                      <ArrowRight size={18} />
                  </button>
                  {step > 1 && (
...
function DateInput({ label, selected, onChange }: any) {
  return (
    <div className="space-y-1.5 sm:space-y-2">
      <label className="text-[9px] sm:text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">{label}</label>
      <div className="relative group">
